...
export default function MovieDetailsPage() {
  ...
  useEffect(() => {
    apiService
      .getInfoById(type, movieId)
      .then((r) => setMovie(r), console.log(type, movieId));
  }, [type, movieId]);

  return (
    <>
      <div className="container">
        <button className={s.btn_back} type="button" onClick={goBack}>
          Back to results{" "}
        </button>
        {movie && <MovieDetails movie={movie} type={type} />}
        <NavLink to="actors" className={s.botLink}>
          Cast
        </NavLink>
        <NavLink to="reviews" className={s.botLink}>
          Reviews
        </NavLink>
